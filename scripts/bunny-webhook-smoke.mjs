...
//
// Synthetic Bunny webhook delivery to verify activation after the operator
// has (a) re-pointed the Bunny dashboard URL at www.furqan.today and
// (b) set BUNNY_WEBHOOK_SECRET in Vercel env.
//
// Usage:
//   BUNNY_WEBHOOK_SECRET="<the secret>" node scripts/bunny-webhook-smoke.mjs
//
// Optional flags:
...
//   --status=<int>           (default 4 = Finished/ready)
//
// Expected outcomes:
...
//   {ok: false, error: "invalid signature"}           → wrong secret
...
const STATUS = Number(args.status ?? 4);
const SECRET = process.env.BUNNY_WEBHOOK_SECRET;

if (!SECRET) {
  console.error(
    "ERROR: BUNNY_WEBHOOK_SECRET env var is required.\n" +
      "Copy it from the Bunny dashboard (Library → Settings → Webhook → Secret).",
  );
  process.exit(2);
}
