...
-- Block 2 — release, PayPal reject, external reconcile. Distinct student per
-- booking to avoid the per-student active-assessment uniqueness.
DO $$
DECLARE
  v_students uuid[]; v_t uuid; v_b uuid; v_req uuid := gen_random_uuid();
  v_res jsonb; v_err boolean := false;
BEGIN
