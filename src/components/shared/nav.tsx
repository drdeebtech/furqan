...
export function Nav({ role, userName }: { role: Role; userName?: string }) {
    ...
    <div dir={dir} className="flex h-full flex-col glass-sidebar">
      ...
      <div className="border-b border-white/10 px-5 py-5">
        ...
        <div className="mt-4">
          ...
          <p className="text-xs text-muted">{lang === "ar" ? ROLE_LABEL[role].ar : ROLE_LABEL[role].en}</p>
        </div>
      </div>
              ...
              {group.links.map((link) => {
                ...
                return (
                  <Link
                    key={link.href}
                    href={link.href}
                    className={`flex min-h-[40px] items-center gap-3 rounded-xl px-3 py-2.5 text-sm transition-colors focus-ring ${
                      active
                  ...
                  >
                    ...
                    {active && (
                      <span className="ms-auto h-5 w-0.5 rounded-full bg-gold" />
                    )}
                  </Link>
                );
              })}
      ...
      <div className="border-t border-white/10 px-3 py-4 space-y-1">
        <a
          ...
          className="flex min-h-[40px] items-center gap-3 rounded-xl px-3 py-2.5 text-sm text-muted transition-colors hover:bg-white/5 hover:text-foreground"
        >
  ...
  return (
    <>
      {/* Mobile top bar */}
      <div dir={dir} className="fixed inset-x-0 top-0 z-40 flex items-center justify-between border-b border-white/10 glass-card px-4 py-2 md:hidden" style={{ borderRadius: 0 }}>
