...
// ─── 1. createCourse ────────────────────────────────────────────────────────
// Returns extra field `courseId` — cannot use loudAction without losing that.
// Keep manual pattern but use UserError for auth checks and logError for DB errors.
...
export const updateCourse = loudAction<
  { courseId: string; formData: FormData },
  void
>({
  name: "courses.updateCourse",
  handler: async ({ courseId, formData }) => {
    const supabase = await createClient();
    ...
    if (ownershipRaw !== null) {
      if (ownershipRaw !== "platform" && ownershipRaw !== "teacher") {
        throw new UserError("نوع ملكية الدورة غير صالح");
      }
      ...
      if (!current) throw new UserError("الدورة غير موجودة");
      ...
      if (ownershipRaw === "platform") {
        ...
        if (!newTeacherId) {
          throw new UserError("اختر المعلم المالك للدورة");
        }
        const { data: teacherRow } = await supabase
          .from("profiles")
          .select("role, deleted_at")
          ...
          .single<{ role: string; deleted_at: string | null }>();
        if (!teacherRow || teacherRow.role !== "teacher" || teacherRow.deleted_at) {
          throw new UserError("المعلم المختار غير صالح");
        }
    ...
    if (error) throw new UserError("فشل تحديث الدورة", { cause: error });
...
export const submitForReview = loudAction<string, void>({
  ...
  handler: async (courseId) => {
    ...
    if (!course) throw new UserError("الدورة غير موجودة");
    if (!["draft", "rejected"].includes(course.status)) {
      throw new UserError("لا يمكن إرسال دورة بحالتها الحالية للمراجعة");
    }
    ...
    if (!lessons || lessons.length === 0) {
      throw new UserError("أضف درساً واحداً على الأقل قبل الإرسال");
    }
    ...
    if (notReady.length > 0) {
      throw new UserError(
        `${notReady.length} درس لا يزال قيد المعالجة — انتظر اكتمالها (قد يستغرق ٢-٥ دقائق بعد الرفع)`,
      );
    }
    ...
    if (course.pricing_type === "one_time") {
      ...
      if (!hasPreview) {
        throw new UserError(
          "الدورات المدفوعة يجب أن تحتوي على درس عرض مجاني (preview) واحد على الأقل",
        );
      }
    }
    ...
    if (error) throw new UserError("فشل إرسال الدورة للمراجعة", { cause: error });
...
export const approveCourse = loudAction<string, void>({
  name: "courses.approveCourse",
  severity: "warning",
  handler: async (courseId) => {
    const supabase = await createClient();
    const admin = (await requireAdminOrMod(supabase)).user;
    ...
    if (error) throw new UserError("فشل الموافقة على الدورة", { cause: error });
...
export const rejectCourse = loudAction<{ courseId: string; reason: string }, void>({
  name: "courses.rejectCourse",
  severity: "warning",
  handler: async ({ courseId, reason }) => {
    const supabase = await createClient();
    const admin = (await requireAdminOrMod(supabase)).user;

    if (!reason.trim()) throw new UserError("سبب الرفض مطلوب");
    ...
    if (error) throw new UserError("فشل رفض الدورة", { cause: error });
...
export const archiveCourse = loudAction<string, void>({
  name: "courses.archiveCourse",
  handler: async (courseId) => {
    const supabase = await createClient();
    ...
    if (error) throw new UserError("فشل أرشفة الدورة", { cause: error });
...
export const deleteCourse = loudAction<string, void>({
  ...
  handler: async (courseId) => {
    ...
    if (error) throw new UserError("فشل حذف الدورة", { cause: error });
