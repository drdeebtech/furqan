...
export function AccountForm({ profile }: Props) {
  ...
  return (
    <form action={formAction} className="grid gap-4 md:grid-cols-2">
      ...
      <div className="md:col-span-2 mt-2 border-t border-[var(--surface-border)] pt-4 text-sm font-medium text-muted">
        بيانات ولي الأمر <span className="me-1 text-xs">(Guardian — optional)</span>
      </div>

      <Field label="اسم ولي الأمر" hint="Guardian name" name="parent_name" defaultValue={profile.parent_name ?? ""} />
      <Field label="جوال ولي الأمر" hint="Guardian phone" name="parent_phone" defaultValue={profile.parent_phone ?? ""} ltr />
      <Field label="بريد ولي الأمر" hint="Guardian email" name="parent_email" defaultValue={profile.parent_email ?? ""} ltr type="email" />
