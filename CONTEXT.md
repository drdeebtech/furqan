...
A *domain action* = a server-side function that mutates a domain's source-of-truth table(s) and emits the canonical events for that domain. Domain actions today are scattered between `src/lib/actions/<domain>.ts` (cross-role) and `src/app/{role}/{feature}/actions.ts` (route-colocated). The Phase 5 pilot (#188) proposes consolidating into `src/lib/domains/<domain>/`.
