...
- **FR-028a**: The acceptance **evidence** fields (`ip`, `user_agent`) MUST be privacy-controlled by **database-enforced** means, not convention:
  ...
  - *Legal hold*: erasure MUST be refused (and audited) while the teacher has an unreleased `payout_holds` row or an active legal-hold flag — destroying consent evidence mid-dispute would remove the platform's own defence.
  - *Retention*: configurable `agreement_evidence_retention_days`, purged by a scheduled job that calls the **same single** erasure function; both branches (erased / refused) write append-only audit rows.
