...
- **FR-028a**: The acceptance **evidence** fields (`ip`, `user_agent`) MUST be privacy-controlled by **database-enforced** means, not convention:
  - *Minimized*: those two fields only — no fingerprinting/geolocation; `user_agent` truncated.
  - *Purpose-limited*: never read by a feature, never exported, never sent to any analytics sink — asserted by a static guard test.
  - *Column-level access*: RLS filters rows, not columns, so it CANNOT protect these fields on a row the teacher may read. `SELECT (ip, user_agent)` MUST be REVOKEd from `authenticated`/`anon`, and in-product reads MUST go through a `security_invoker` safe view that omits both columns. Raw access is service-role only, for a documented legal request.
  - *Erasure without breaking append-only*: the immutability trigger MUST reject every UPDATE **except** one narrow exception — a `SECURITY DEFINER` erasure function that nulls **only** `ip`/`user_agent` (all other columns provably unchanged) and only when the legal-hold predicate passes. **The consent record itself is never erasable**: the platform must always be able to prove consent existed, even after the capture evidence is purged.
  - *One erasure predicate*: evidence is erasable **only** when ALL hold — (1) no earning stamped with that `agreement_version` is still in flight (status not in `transferred|manual_paid|voided|debt_recovered`), (2) the teacher's `outstanding_debt_cents = 0` (we are still deducting under these terms, so their consent evidence must survive), (3) `accepted_at + agreement_evidence_retention_days < now()`, and (4) no legal hold (no unreleased `payout_holds` row, no active `legal_hold_until`). The predicate is defined **once**, executably, in plan Phase 0 and is the only answer to "is this erasable?". The canonical association is a **stamped** `agreement_version` on the earning entry (FR-030a), never a timestamp derivation — the latter is ambiguous exactly at a version boundary.
  - *Erasure function lockdown*: the `SECURITY DEFINER` erasure function MUST pin `search_path` and, in the same migration, `REVOKE EXECUTE ... FROM public, anon, authenticated` and `GRANT EXECUTE ... TO service_role` — revoking from `public` alone leaves it callable (this repo's spec-016 finding). Negative test: an `authenticated` caller gets `insufficient_privilege`.
  - *Retention*: configurable `agreement_evidence_retention_days`; the scheduled purge and any ad-hoc erasure call the **same single** function; both branches (erased / refused) write append-only audit rows.
  ⚠ The retention **period** is a legal/owner input — engineering pins the mechanism and the setting, not the number.
...
- **FR-030a**: A new agreement **version** MUST require re-acceptance before the teacher's next booking, and MUST NOT retroactively alter already-accrued earnings or already-settled transfers. The governing version is the one accepted at accrual time, and it MUST be **stamped** on the earning entry at materialization — **not** derived by comparing timestamps, which is ambiguous exactly at a version boundary (the moment it matters) and would make the evidence-retention predicate (FR-028a) unanswerable. Assert in tests.
...
- **Teacher Debt / Negative Balance**: the teacher's running balance MAY be negative (negative carryover). **One definition, shared by every module** (see plan Phase 0 for the worked example): every row carries a signed `amount_cents` — `clawback` negative, `debt_recovery` positive, `debt_recovery_reversal` negative — and `outstanding_debt_cents = GREATEST(0, -1 × SUM(amount_cents) FILTER (WHERE kind IN ('clawback','debt_recovery','debt_recovery_reversal')))`. Earning rows are **not** part of this sum; they are what pays it. (Never `sum(clawback) − sum(recovery)`: with signed amounts that double-counts and inverts the balance.) Automatically offset against future earnings before any transfer (FR-014), never collected by any other means, and always reconstructable from append-only rows.
