...
4. `teacher_agreement_acceptances`: `teacher_id`, `agreement_version text`, `accepted_at`, `accepted_by`, `ip inet NULL`, `user_agent text NULL`; append-only (no UPDATE/DELETE policy at all), service_role insert, teacher SELECT own, admin SELECT (spec FR-028). Unique on `(teacher_id, agreement_version)`.
   **Privacy controls for the evidence fields (`ip`, `user_agent`) — spec FR-028a. These are DB-enforced, not conventions**:
...
5. `teacher_earning_entries`: **one canonical source-key model — every payable unit has exactly one unique source key**: partial `UNIQUE(session_delivery_id) WHERE kind='session'` **and** partial `UNIQUE(payment_id) WHERE kind='course'`, so re-running materialization can never duplicate an earning from either source (FR-008 alignment). Plus `teacher_id`, `amount_cents int` (negative allowed only for `kind='clawback'`), `funding_charge_id text NULL`, `transfer_group text`, `status` enum `pending|processing|held|transferred|voided|debt_recovered|manual_due|manual_paid`, `hold_reason`, `claimed_at timestamptz NULL` (sweep lease — Phase 1), `external_reference_id text NULL` + `settled_by uuid NULL` + `settled_at timestamptz NULL` (manual rail, spec FR-027; partial `UNIQUE(teacher_id, external_reference_id)` to catch a pasted-twice reference), timestamps.
   ...
   **THE sign convention — one definition, stated once, used everywhere** (spec Key Entities and the Phase 1 property test MUST cite this and nothing else; a second phrasing is how a ledger silently inverts):
   - Every row carries a **signed** `amount_cents`. `clawback` is **negative** (debt created). `debt_recovery` is **positive** (debt paid down). `debt_recovery_reversal` is **negative** (a recovery undone — debt restored).
   - **`outstanding_debt_cents = GREATEST(0, -1 * SUM(amount_cents) FILTER (WHERE kind IN ('clawback','debt_recovery','debt_recovery_reversal')))`**, per teacher. Earning rows (`session`/`course`) are **not** part of this sum — they are the source that pays the debt, not the debt itself. Worked example: clawback `-100` → debt 100; recovery `+40` → debt 60; that recovery reversed `-40` → debt 100 again. Never subtract recovery rows from clawback rows: with signed amounts that double-counts and inverts the balance.
   - `consuming_entry_id` (FK → the earning entry whose payout funded the recovery) and `recovered_against_entry_id` (FK → the `clawback` row being paid down). Both NOT NULL for `kind='debt_recovery'`, both NULL for every kind except the reversal below (CHECK-enforced).
   ...
   - `teacher_id` denormalized (as on every row) so the balance is a single-table aggregate with no joins.
   ...
   **`debt_recovery_reversal` — full contract** (FR-011; a compensation with no replay key is just a second way to corrupt the balance):
   - Part of the `kind` domain: `session|course|clawback|debt_recovery|debt_recovery_reversal` (the enum/CHECK MUST list it — it is not prose).
   - `reverses_recovery_id` (FK → the `debt_recovery` row being undone), NOT NULL for this kind and NULL for every other (CHECK-enforced). `amount_cents` MUST equal `-1 × reversed.amount_cents` (CHECK or trigger) — a partial un-recovery is not a thing.
   - Replay key: partial **`UNIQUE(reverses_recovery_id) WHERE kind='debt_recovery_reversal'`** — at most one reversal per recovery, so a retried failure path can never append duplicate compensations.
   - **Atomic retry semantics**: the failure path (`processing → pending`, FR-011) MUST write the reversal in the **same transaction** as the status flip. Either both land or neither: an entry back in `pending` with its recovery still applied would pay the debt twice, and a reversal without the flip would restore debt for an entry that still transfers. The next sweep then re-derives the netting from scratch against the restored `outstanding_debt_cents` — no state is carried between attempts.
   - Never delete or mutate the original recovery row; the ledger stays append-only and every attempt is reconstructable. Financial-columns immutable trigger (copy `guard_teacher_payouts_financials` idiom); status transitions constrained by trigger (e.g. `transferred` and `manual_paid` are terminal except clawback rows).
...
4. `debt.ts`: pure `netAgainstDebt(earningCents, outstandingDebtCents) → { transferCents, recoveredCents, remainingDebtCents }` (integer cents; `transferCents = max(0, earning − debt)`) + orchestration that writes the `debt_recovery` ledger row **inside the same atomic claim** as the entry it consumes (spec FR-014). One shared `outstandingDebt(teacherId)` reader implements the Phase 0 sign convention — **no module re-derives the balance with its own formula**. Property tests: recovery never exceeds the earning; the reconstructed balance always equals the Phase 0 definition (`GREATEST(0, -1 * SUM(amount_cents))` over the debt kinds) for random row sequences; replaying a sweep recovers exactly once; a failed transfer's reversal restores the balance to its pre-recovery value **exactly** (round-trip property).
...
| **Admin flips `manual → stripe_connect` while an entry is already `manual_due`** → the `connect_settle_manual_due` `payout_method='manual'` guard makes the settle a permanent no-op, and the sweep won't re-claim it (it only claims `pending`) → entry stranded | Correct *refusal* (never hand-settle money that must flow through Stripe), but recovery is a **Phase 4 admin-ops** concern: the FR-025 rail switch must surface stuck `manual_due`/`manual_paid`-mismatched entries and offer an admin re-route (e.g. requeue to `pending`), audit-logged |
