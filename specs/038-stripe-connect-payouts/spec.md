...
- **Teacher's Connect account is rejected/disabled by Stripe** → entries stay `pending` indefinitely; ops alert; manual-payment fallback remains possible (the entry ledger is payment-method-agnostic).
...
- **Teacher in a country Stripe Connect doesn't support** → onboarding fails at Stripe's hosted page; status stays `onboarding_incomplete`; entries accrue as `pending`; ops sees it. ⚠ **HUMAN REVIEW (policy)**: alternative payment rail for these teachers is out of scope here but must be decided before the manual process is retired.
...
- **FR-006**: Earning amount per delivery MUST be `round(duration_minutes / 60 × hourly_rate_usd × 100)` integer cents from the **snapshotted** `session_deliveries.hourly_rate_usd` — never a live rate re-read. Course-purchase earnings MUST reuse `computeCourseRevenueSplit` unchanged.
...
- **FR-021**: A cutover date (platform_settings) MUST partition history: deliveries dated before it remain payable only via the legacy `run_monthly_payroll` path; deliveries on/after it are payable only via Connect transfers. No delivery may be payable by both (assert in tests).
...
- **Earning Entry** (`teacher_earning_entries`): one row per payable unit (session_delivery_id UNIQUE for sessions; course-purchase entries keyed on payment id). Amount in integer cents, snapshotted. States: `pending → held → transferred | voided`; plus negative `clawback` rows. Financial columns trigger-immutable.
