...
4. **Idempotency that survives a partial failure.** The authoritative, *resumable* dedupe is the **per-student stamp**: after each student's dispatch, stamp `last_intervention_at=now(), intervention_type='reengagement_7d'`. Because detection filters out anyone stamped within the cooldown window, a retry after a mid-batch crash naturally **skips already-nudged students and resumes the rest** — no student is double-nudged, none is skipped. The daily `automation_logs` row (`idempotency_key='reengagement-nudge-<YYYY-MM-DD>'`) is a **completion/observability marker written only after the batch finishes** (status `succeeded`), NOT a start-of-run gate — writing it first would let a partial failure suppress every same-day retry, turning a transient outage into a missed run (CodeRabbit #603). If a `running`/failed marker already exists for today, resume rather than abort.
