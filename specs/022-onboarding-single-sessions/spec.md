...
- **FR-014**: Assessment availability MUST be constrained per student to prevent free/cheap assessment farming, per a configured policy. *(RESOLVED 2026-06-16: per-specialty limit via `hifz_assessment_limit_per_specialty`; an over-limit request is rejected with **409** before any charge — contracts §1.)*
