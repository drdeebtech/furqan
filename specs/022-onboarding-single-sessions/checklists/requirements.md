...
- **Validation status**: PASSED with 3 open clarifications. Run `/speckit.clarify` to resolve them before planning: (1) default assessment price / free-at-launch, (2) per-student assessment frequency limit (anti-farming), (3) whether specialized purposes + assessment specialty taxonomy are a new enumerated set or reuse existing `session_type`/teacher `specialties`. None block the overall shape; all three are bounded, low-risk decisions.
