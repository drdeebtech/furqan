...
- **Cutover DATE/TIME policy (DECIDED):** The cutover window is the first Sunday at 02:00–04:00 UTC that falls at least 30 days after spec 023 confirms live in production. The Platform Owner sets the exact timestamp, commits it to the runbook at T001, and pre-announces it to teachers and students at least 72 h in advance. The timestamp must be expressed as an absolute UTC string (e.g. `2026-09-07T02:00:00Z`) — no relative dates in the runbook.
- **Legacy-balance→new-entitlement conversion policy (DECIDED):** Active `student_credits` rows (`used < total` AND (`expires_at IS NULL` OR `expires_at > cutover_timestamp`)) are converted to new `student_packages` grants: `sessions_total = sessions_remaining = (total - used)`, `billing_cycle_key = 'legacy_migration_' || id::text` (idempotent anchor), `subscription_id` linked to the student's new subscription if one exists, `teacher_id` preserved if set. Expired credits (`expires_at <= cutover_timestamp`) and zero-balance credits (`used = total`) are NOT migrated — they lapsed. No partial-session math: `student_credits.total` and `.used` are integer session counts so `remaining` is always a whole number. The migration log records every converted and skipped row for audit. `student_packages` rows that already exist in the new schema (from spec 018/019 grants) are never touched — the migration only INSERTs legacy-source rows keyed on `billing_cycle_key LIKE 'legacy_migration_%'`.
- **Rollback decision authority (DECIDED):** The Platform Owner (account `drdeebtech`) is the sole person authorized to invoke rollback at any stage. Go/no-go at each verification gate requires explicit Platform Owner sign-off (a recorded confirmation in the runbook channel — Slack/email/call). No automated or delegated rollback is permitted; the rollback script requires a manual `CONFIRM_ROLLBACK=yes` environment flag to execute.
- **Captured-live-payments rollback policy (DECIDED):** If rollback is invoked within 24 h of the Stripe live flip (FR-021), all charges captured after the flip timestamp are HELD — no automatic refunds. The Platform Owner reviews each held charge within 48 h: charges where the subscription service was not yet delivered are refunded via Stripe dashboard; all others are honored. Rollback invoked MORE THAN 24 h after the live flip is DISALLOWED — the system is considered committed at that point (too many live charges to safely unwind). The rollback script enforces this: it reads the flip timestamp from the runbook log and aborts with a clear error if `now() - flip_timestamp > interval '24 hours'`. This policy is the authoritative source; the fail-closed guard in the rollback path is satisfied by this decision.
