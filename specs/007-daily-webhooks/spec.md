...
- Q: When `meeting.ended` lands and `sessions.started_at` is still NULL (out-of-order or dropped `meeting.started`), what happens? → A: Fill `started_at = end_time - duration` from the same payload (Daily-canonical means Daily's math is authoritative). The session row is immediately complete and the booking flips. No second roundtrip waiting on a possibly-never-arriving `meeting.started`.
- Q: When a webhook event maps to a session whose `bookings.status` is `cancelled` or `no_show`, what happens? → A: Write `sessions.started_at`/`ended_at`/`actual_duration` honestly (audit trail stays accurate about what happened on Daily's side), but NEVER flip `bookings.status` away from `cancelled` or `no_show`. The booking domain owns cancellation; a stray Daily event MUST NOT override a deliberate user action. Audit log records `session.webhook.ended_on_cancelled` for forensics.
- Q: What `duration` threshold counts as a "real lesson" worth flipping the booking to `completed`? → A: 5 minutes (300 seconds). Below the threshold, `sessions` is written honestly (audit trail) but the booking flips to `no_show` instead of `completed` — preventing 15-second misclicks from debiting student packages and spamming parent reports. Filter is necessary at 50k DAU where accidental room joins are routine.
- Q: When the misclick filter flips a booking to `no_show`, which downstream side effects fire? → A: The route adapter emits `session.no_show` (existing event in `WEBHOOK_ROUTES`) instead of `session.ended`. Reuses existing n8n routing — gentler parent copy ("we noticed the call didn't happen; rescheduling options below") and the package stays unbeducted because `deduct_package_session` is wired to `session.ended` only. The `completed` and `reconciled` paths still emit `session.ended`; the `ended_on_cancelled` path emits nothing (booking-domain ownership preserved).
- Q: Should `sessions.room_name` carry a UNIQUE constraint, and what happens if a webhook arrives for a duplicate? → A: Add a partial UNIQUE constraint: `UNIQUE (room_name) WHERE room_name IS NOT NULL`. Daily-side room names ARE globally unique by construction (via `createRoom`); the partial UNIQUE just enforces the invariant at our DB so a future `createRoom`-retry bug, manual SQL insert, or migration mistake surfaces at INSERT time (caller sees an error) instead of at webhook time (silent wrong-row mutation). The partial form lets `room_name=NULL` rows coexist during the backfill window.
- Q: Should the receiver enforce a max time skew between Daily's `end_time` / payload `timestamp` and server `NOW()`? → A: Yes, ±15 minute window. Events with payload timestamp more than 15 minutes ahead or behind `NOW()` are rejected with 200 + `applied: false, reason: "stale-event"` (so Daily does not retry) and logged at `warning` severity. Catches clock-drift bugs and quietly drops belated retries from long Daily outages without rejecting legitimate retries during a normal 5-min outage. Replay attacks are already mitigated by FR-002 idempotency on `event_id`; this skew check is purely the clock-drift / stale-queue detector.
...
- **FR-001**: System MUST verify every inbound Daily.co webhook payload against the configured shared-secret HMAC signature before reading any field from the body. Verification MUST attempt the current `DAILY_WEBHOOK_SECRET` first, then — if an optional `DAILY_WEBHOOK_SECRET_PREVIOUS` env var is set — the previous secret. The previous-secret path MUST be removable by clearing the env var after a 24-hour overlap window. Failed verification under both secrets MUST return 401 and MUST NOT touch the database. After successful signature verification, system MUST enforce a ±15-minute skew window between **`payload.timestamp`** (the top-level epoch-ms delivery-time field from Daily, NOT the `data.start_time` or `data.end_time` meeting boundaries) and server `NOW()`; events outside the window MUST return 200 with `applied: false, reason: "stale-event"` (so Daily does not retry) and MUST be logged via `logError(severity: "warning", tag: "daily-webhook")` only — stale events MUST NOT be inserted into `audit_log`, matching the unmappable-room handling policy. The idempotency check (FR-002) already mitigates replay attacks on `event_id`; this skew check is purely a clock-drift and stale-queue detector.
...
- **FR-005**: On `meeting.ended`, system MUST set `sessions.ended_at` to Daily's reported timestamp, set `sessions.actual_duration` to the duration Daily reports, and — if the corresponding `bookings.status` is `confirmed` AND Daily's `duration >= 300` seconds — flip it to `completed`. If `duration < 300` seconds, the booking flips to `no_show` instead (filters misclicks at 50k-DAU scale; the `sessions` row still records the brief activity for audit). If the booking is already `completed`, system MUST leave the status unchanged but still update `ended_at` and `actual_duration`. If `bookings.status` is `cancelled` or `no_show`, system MUST still update `sessions` (for accurate audit trail) but MUST NOT flip the booking status; the audit log records action `session.webhook.ended_on_cancelled` for forensic visibility. If `sessions.started_at` is `NULL` at the time `meeting.ended` is processed (out-of-order or dropped `meeting.started` event), system MUST retroactively fill it via `started_at = end_time - duration` from the same payload — Daily's `end_time - duration` is authoritative.
- **FR-006**: Webhook receiver MUST acknowledge each event with a 2xx response within 500ms (P99). The handler's hot path MUST be limited to: HMAC verification → idempotency check → `sessions` row update → `bookings.status` flip → fire-and-forget event emission. Event selection follows the SQL function's `status_outcome` return: `completed` → emit `session.ended`; `no_show` (misclick filter) → emit `session.no_show`; `preserved` (cancelled/no_show booking) → emit nothing (booking-domain ownership). All downstream side effects (parent notifications, package-deduction confirmation, n8n routing) MUST flow through the existing `emitEvent` pattern in `src/lib/automation/emit.ts` and execute off the request path.
...
- **FR-009**: System MUST tolerate webhook bursts of up to 500 events in a 60-second window without dropping events or exceeding 500ms P99 acknowledgement latency. The release load test (T018) verifies the lower bound of 200 events/60s — the 500 figure is the design ceiling that gives a 2.5× safety margin over peak (Friday-evening hifz cohort) projected at 50k DAU. Receiver MUST NOT perform per-event synchronous Supabase queries that scale linearly with payload size.
...
- **Audit log entry**: existing `audit_log` table. New action codes: `session.webhook.started`, `session.webhook.ended`, `session.webhook.duplicate`, `session.webhook.unmapped`, `session.webhook.reconciled` (when webhook overwrites a manual end), `session.webhook.misclick_filtered` (Daily duration < 5 min — booking flipped to `no_show` instead of `completed`), `session.webhook.ended_on_cancelled` (webhook arrived for a booking the user had already cancelled or marked no_show — `sessions` row updated for audit trail, booking status preserved).
...
- **SC-005**: Operator can detect and react to any cluster of webhook failures (HMAC mismatch, unmappable room) within 5 minutes of onset, via a direct Telegram alert path triggered by `logError(severity: "warning", tag: "daily-webhook")` that bypasses the hourly Sentry-watcher cron. End-to-end verification: see T019.5 in `tasks.md`.
