...
- [x] CHK004 Is the behavior of `run_monthly_payroll` for a teacher with zero delivered hours specified as a definite outcome (no row vs zero-value row), rather than left as "or"? [Completeness, RESOLVED 2026-06-16 — spec Edge Cases now states definitively **no payout row** (aggregation groups over `session_deliveries`, so no delivered sessions → no row), not an error; the "or a zero-value row" ambiguity removed.]
- [x] CHK005 Is the required handling of a missing or zero hourly rate at payroll time specified (fail loud / configuration error), and is it stated as a testable requirement rather than only an edge-case note? [Completeness, RESOLVED 2026-06-16 — promoted from edge note to **FR-030**: `run_monthly_payroll` skips a NULL/0-rate teacher/month and surfaces it as an exception (no silent `$0`); testable via SC-009, data-model.md `run_monthly_payroll` guard, contracts §5 `exceptions[]`, tasks T026/T008, quickstart Scenario 6.]
...
- [x] CHK046 Is the `MAX(hourly_rate_usd)` aggregation in `run_monthly_payroll` reconciled with the snapshot model — i.e., is it stated/guaranteed that all rows for a teacher/month share one rate, so MAX cannot silently mask a mid-month rate change? [Ambiguity, RESOLVED 2026-06-16 — **FR-029** makes the constant-rate-per-teacher/month an explicit, testable invariant; data-model.md `run_monthly_payroll` now detects non-uniform rates (`MIN <> MAX`) and skips+surfaces them rather than silently selecting `MAX`. Covered by SC-010, tasks T006/T026/T008, contracts §5 `exceptions[]`, quickstart Scenario 6.]
