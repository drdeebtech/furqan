...
- **Subscription-Period Extension** (`subscription_extensions` table, introduced Phase 0): one row per excused-carried session that earned an extension — `id`, `subscription_id` FK, `granted_by_user_id` FK, `reason`, `extension_seconds bigint`, `booking_id` FK (idempotency anchor — always present), `session_id` FK (nullable audit link), `granted_at`. Effective access end = `subscriptions.current_period_end + SUM(extension_seconds)`. The mirror column is never mutated.
