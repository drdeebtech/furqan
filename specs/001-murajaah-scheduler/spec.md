...
### v0 → v1 framing (added 2026-05-08 after PR #218 review)

This spec describes a **v1 rewrite** of an existing v0 implementation that is already shipped:

- **Existing v0 cron**: `src/app/api/cron/murajaah-due/route.ts` (n8n-triggered at 09:00 UTC daily). Uses `study_log` for engagement; sends a generic `notify(type='system')` reminder to active students who haven't logged study today. No SM-2, no per-row scheduling, no review batches.
- **Existing v0 dashboard card**: `src/app/student/dashboard/murajaah-card.tsx` reads `MurajaahWindow` (yesterday / last week / last month) from `dashboard-queries.ts`. Displays three time-window prompts; CTA links to `/student/time-tracker`. No mark-complete buttons; no SM-2 due-date math.
- **Existing v0 engagement signal**: `study_log` rows. Distinct from this spec's behavioural `last_reviewed_at` check on `student_review_schedule`.

The v1 (this spec) replaces v0's data model and engagement logic. The v0 cron MUST be retired in the same PR that ships v1 (see new FR-014). The v0 `MurajaahCard` component is replaced; the file path may stay or change at the implementer's discretion as long as no two cards render simultaneously.

Why this acknowledgement matters per Constitution Principle V (Tracer-Bullet Adoption): a v0→v1 rewrite is *not* a tracer-bullet pilot. It carries higher risk than a greenfield feature because two implementations can briefly coexist and confuse users. The mitigation is Step 1 of the Phase 2 task plan: ship v1 *and* retire v0 in the same migration window.
...
- Q: How is "opened the app today" defined for the FR-005 nudge? → A: Behavioural — "marked at least one Murajaah review complete today, in the student's local timezone" (Option C). Cron suppresses the 7pm nudge if `EXISTS (SELECT 1 FROM student_review_schedule WHERE student_id = X AND (last_reviewed_at AT TIME ZONE student_tz)::date = (now() AT TIME ZONE student_tz)::date)`. The "today" date is the **student-local-timezone date everywhere in this spec** — never UTC. Decision rationale at 50k user scale: zero new writes (vs. Option B's ~250k profile UPDATEs/day from per-dashboard-render timestamping), zero new schema, uses an existing indexed column. Correctly nudges the opens-but-doesn't-click group, which is exactly the audience the 7pm reminder is designed for. Background PWA pings do not affect this signal.
...
- **FR-005**: System MUST send a notification (in-app + dispatcher channel preferences per `communication_preferences`) when a student has ≥ 3 due reviews and has NOT marked any Murajaah review complete today. The "engaged today" check is `EXISTS (SELECT 1 FROM student_review_schedule WHERE student_id = X AND (last_reviewed_at AT TIME ZONE p.timezone)::date = (now() AT TIME ZONE p.timezone)::date)` (where `p` is the student's `profiles` row) — purely behavioural; opening the app without clicking does NOT suppress the nudge. Per Principle III this notification is best-effort post-commit; the nightly cron computes the eligible cohort, the dispatcher fires at the cohort's local 7pm window respecting `communication_preferences` quiet-hours.
...
- **FR-008**: The compute step (FR-001) MUST run inside a Postgres function (per Principle III) since it touches `student_progress` and a new `student_review_schedule` table atomically. The function is invoked nightly at 02:00 UTC by an n8n workflow (registered in `automation/BLUEPRINT.md`). The cron iterates students and writes `batch_for_date` as **the student's local-timezone date for tomorrow**, not the UTC date — so a student in UTC-5 whose local "tomorrow" started at 05:00 UTC reads the row that the cron wrote when it processed that student. The function MUST be idempotent — safe to re-run within the same UTC tick without producing duplicate batch rows for any student-local date.
...
- **FR-012**: The student dashboard MUST read pre-computed rows where `batch_for_date = (now() AT TIME ZONE student_tz)::date`. The "today" date is the **student's local-timezone date**, never UTC. No on-the-fly compute on dashboard render. If the cron has not yet produced a row for the student's current local date (e.g., the nightly job failed, or the student is in a timezone the last cron tick has not yet "reached"), the dashboard MUST fall back to displaying yesterday's leftover unmarked items rather than an empty card; n8n healthcheck flags any actual missed cron run separately.
- **FR-013**: The teacher panel (User Story 2) MUST surface a "needs reteaching" section listing items where the student's `next_review_at` is 8+ days overdue. The teacher can mark items as reteached after a session, which resets the row to a fresh schedule (`lapse_count++`, `easiness_factor` reduced per SM-2 lapse penalty, `next_review_at` = today + 1 day, where "today" is the student's local-timezone date).
- **FR-014**: The v1 implementation MUST retire the v0 cron at `src/app/api/cron/murajaah-due/route.ts` and the v0 `MurajaahCard` component (`src/app/student/dashboard/murajaah-card.tsx`) in the same PR that ships v1. Concretely: the v0 cron route file is deleted; the v0 component is either deleted or rewritten in place to render the v1 batch; the n8n workflow triggering the v0 cron at 09:00 UTC is disabled before the v1 02:00 UTC workflow is enabled. No transition window where both crons run; no two `MurajaahCard` components rendering simultaneously. This requirement exists because PR #218 review surfaced the existing v0 implementation that the original draft of this spec did not acknowledge — the constitution Principle V violation (v0→v1 rewrite is not a tracer-bullet pilot) is mitigated by the same-PR retirement contract.
- **FR-015**: All "today" date comparisons in this spec — engagement check (FR-005), batch date filter (FR-012), reteach `next_review_at = today + 1` (FR-013) — MUST use the student's local-timezone date computed via `(now() AT TIME ZONE student_tz)::date`. The student timezone source is `profiles.timezone` (existing column, default `Asia/Riyadh`). Postgres `current_date` (UTC) MUST NOT appear in any Murajaah query for date comparisons.

### Key Entities

- **student_review_schedule** — one row per `(student_id, progress_id)` storing `next_review_at`, `easiness_factor`, `lapse_count`, `last_reviewed_at`, `algorithm_version`, `batch_for_date` (**the student's local-timezone date** this row was scheduled into the cached daily batch by the nightly cron, or NULL if not in any cached batch — never a UTC date). Owned by the **Progress** domain (per CONTEXT.md "Domains").
...
- Per-region cron triggers — v1 uses one 02:00 UTC tick that iterates students by timezone within Postgres. If telemetry shows a meaningful tail (e.g., 8% of students in UTC+10 see batches generated 14h late), promote to per-region triggers in v2.
- Backfilling old `study_log` rows into `student_review_schedule` — v0 used `study_log` for engagement; v1 uses `last_reviewed_at` on schedule rows. Old `study_log` rows are not migrated; the cohort gets fresh schedules on next cron tick. Acceptable because Murajaah is a forward-looking habit feature; historical "study completed" events don't translate cleanly to "review completed."
