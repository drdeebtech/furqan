...
1. **Given** a `payment.failed`/dunning event from 018, **When** notification routing runs, **Then** a pre-suspension alert is delivered once per recipient on the configured channels, recorded in the idempotency ledger.
...
- **FR-005**: System MUST issue a **simple appreciation certificate** when a student completes a juz, and after any defined level milestone, as encouragement. The certificate MUST cite the exact juz / `surah:ayah` range completed, validated against canonical structure; an unvalidatable range MUST fail loudly rather than render an approximate value. **Juz-boundary source (clarified 2026-06-19, round 2):** canonical juz→surah:ayah mappings MUST come from `src/lib/quran/juz-boundaries.ts`, authored by an architect transcribing from a cited canonical mushaf (NOT model-generated — AGENTS.md §2). Until that file ships, the juz branch of FR-005 (and T014/T015 juz paths) is blocked; the level-milestone branch is unaffected.
...
- **FR-012**: System MUST own the **content** (Arabic-first, RTL) for each notification trigger it serves: dunning/pre-suspension alert, expiry "continue?" prompt, payment-retry/dunning escalation, absence/excuse outcome, monthly-report-ready, and certificate-earned. Each trigger MUST resolve to a **defined default channel set** (the `channel text[]` written on the `notifications` row), per the matrix below. These defaults are **admin-configurable** and live in `platform_settings` under key `notification_channel_matrix` (a JSON map of `trigger → channel[]`, validated against the widened channel set {in_app, email, push, whatsapp}); when no override is present the trigger falls back to its default below. They are NOT hardcoded in handler code.
  ...
  **Trigger → default channel matrix** (subset of {in_app, email, whatsapp}; `push` reserved, off by default):
  ...
  |---------|------------------|-----------|
  | `payment.failed` (dunning / pre-suspension) | `in_app`, `email`, `whatsapp` | money/continuity-critical — highest-reach |
  | `subscription.expiring` (expiry "continue?") | `in_app`, `email`, `whatsapp` | renewal prompt, sent 7d before period end (configurable) — highest-reach |
  | `payment_retry` / dunning escalation | `in_app`, `email`, `whatsapp` | escalation of an unresolved failure |
  | `absence.outcome` (excuse / make-up) | `in_app`, `email` | informational outcome — no WhatsApp by default |
  | `monthly_report.ready` | `in_app`, `email` | recurring digest — no WhatsApp by default |
  | `certificate.earned` | `in_app` | in-app delight moment; email optional via override |
...
- **FR-013**: For each owned trigger, the system MUST consume the **event emitted by the owning spec** — except where this spec owns emission per the **round-2 clarification**: spec 023 consumes the existing `subscription.past_due` event (already emitted by spec 018's webhook) for the dunning / payment-failed flow, and **emits `subscription.expiring` and `absence.outcome` locally** (no upstream emitter exists for these as of 2026-06-19). Specs 018/021 may later take over emission if/when they ship those events; the dot.notation names already match. The expiry "continue?" prompt MUST be sent at exactly `period_end - N days` where N defaults to 7 and is admin-configurable via `platform_settings.subscription_expiring_lead_days`, and `period_end` is read from **`subscriptions.current_period_end`** (single source of truth per spec 018).
- **FR-014**: Notification delivery MUST be **idempotent** per (recipient, trigger, subject) via the existing `automation_logs` unique idempotency key, with status one of `started/succeeded/failed/skipped`; a duplicate/replayed trigger MUST resolve to `skipped`. **Retry-vs-lock semantics (clarified 2026-06-19):** `failed` is **non-terminal** — the UNIQUE constraint on `idempotency_key` is a partial index `WHERE status <> 'failed'`, so a `failed` row releases the lock and the next delivery of the same event re-attempts (started → succeeded/failed again). `succeeded`, `skipped`, and in-flight `started` rows still lock. Required for SC-006's retry-safe guarantee. **Canonical `automation_logs` columns (clarified 2026-06-19, round 2):** use the actual schema — `workflow_name` (NOT NULL), `event_name` (nullable), `payload_json` (jsonb), `result_json` (jsonb), `error_message` (text), `status` (CHECK in `started/succeeded/failed/skipped`), `idempotency_key` (UNIQUE), `attempt_count` (default 1), `entity_type`/`entity_id` (nullable), `channel` (nullable). No `event_type` / `payload` / `error_detail` columns exist; do not invent them.
...
- **FR-017**: Event names used for emission/consumption MUST come from the **shared typed event surface** (AGENTS.md §4 — `FurqanEvent`/the shared `Events` enum), never string literals introduced ad hoc. Spec 023's complete event surface (owned + consumed, all dot.notation per the codebase convention in `src/lib/automation/emit.ts`):
  - **Owned + emitted by this spec**: `monthly_report.ready`, `certificate.earned`, `honor_board.updated`.
  - **Consumed from spec 018**: `subscription.past_due` (already emitted).
  - **Emitted locally by this spec** (no upstream emitter as of 2026-06-19): `subscription.expiring`, `absence.outcome`.
  - **Consumed from spec 018 (pending)**: `subscription.month_closed` (no emitter yet — ⛔ T011a blocker; T011b fallback detector).
...
- **SC-005**: Each owned notification trigger delivers on **exactly the channel set defined by the FR-012 matrix** (or its `platform_settings` override) for **100%** of recipients — verified by asserting the `notifications.channel[]` array equals the matrix default for each trigger (e.g. `payment.failed` → {in_app, email, whatsapp}; `absence.outcome` → {in_app, email}; `certificate.earned` → {in_app}), with WhatsApp dropped when `notifications_whatsapp_enabled='false'`. The expiry "continue?" prompt MUST be sent at exactly `period_end - 7d` unless `subscription_expiring_lead_days` is overridden in `platform_settings`.
...
- Q: "Report ready" / "certificate earned" notification idempotency key prefix (CHK048) — `notif:` or reuse `report:`/`cert:`? → A: **Distinct `notif:` prefix.** Notification delivery is a separate concern from artifact issuance (either can fail independently). `monthly_report.ready` → `notif:{guardianId}:monthly_report.ready:{reportId}`; `certificate.earned` → `notif:{recipientId}:certificate.earned:{certId}`. Issuance keys (`report:{studentId}:…`, `cert:{studentId}:…`) stay distinct. Sharing would create a false cross-dependency: a successful issuance + failed notification would block retry of either.

### Session 2026-06-19 (speckit-clarify round 2 — implementation blockers)

Five ambiguities surfaced during `/speckit-implement` (Phase 1+2). The first three were hard blockers; the last two are high-impact follow-ups. All answers selected on best-practice / pragmatic-ownership grounds.

- Q: Consumed-events ownership — specs 018/021 never emitted `payment.failed`/`subscription.expiring`/`absence.outcome`. Who owns emission? → A: **Pragmatic split (Option C).** Spec 023 consumes the existing `subscription.past_due` event (already emitted by spec 018 webhook) for the dunning / payment-failed flow. Spec 023 emits `subscription.expiring` and `absence.outcome` locally (the two with no upstream emitter). Specs 018/021 can later take over emission if/when they ship those events; the dot.notation names already match the codebase convention. Unblocks US1/US5 webhook wiring (T012/T029/T030) without waiting on cross-team coordination.
- Q: `automation_logs` canonical column names — spec assumed `event_type`/`payload`/`error_detail`; actual schema differs. → A: **Use the actual schema.** Required fields: `workflow_name` (NOT NULL), `event_name` (nullable), `payload_json` (jsonb), `result_json` (jsonb), `error_message` (text), `status` (CHECK in `started/succeeded/failed/skipped`), `idempotency_key` (UNIQUE), `attempt_count` (default 1), `entity_type`/`entity_id` (nullable), `channel` (nullable). No `event_type` / `payload` / `error_detail` columns exist. All domain code (issuance, routing, the spec-local delete-and-retry mechanism from the prior session's Q1/CHK032) MUST use these canonical names.
- Q: Quran juz-boundary canonical source (AGENTS.md §2 hard constraint — no model-generated Quran data). → A: **Architect transcribes from a verified canonical mushaf.** Author `src/lib/quran/juz-boundaries.ts` mirroring the existing `ayah-counts.ts` pattern: 30 entries, each `juz N → { start_surah, start_ayah, end_surah, end_ayah }`, sourced from a cited canonical reference (e.g., the official Madani King Fahd Complex print). The file MUST include a `TOTAL_JUZ = 30` self-check that throws at module load if the array length drifts (same integrity pattern as `TOTAL_AYAT = 6236`). No model generation; transcription from a cited source only. Until this file ships, US2 juz-certificate issuance (FR-005 juz branch + T014/T015) is blocked. Level/course-certificate branches (no juz data needed) remain unblocked.
- Q: T011b fallback month-close detector — when to ship? → A: **Ship only if spec 018 hasn't emitted `subscription.month_closed` by US1 ready date** (current plan). Minimal scope: the fallback exists to unblock, not as defense-in-depth. Once spec 018 ships the emitter, T011b is deleted (the task already says so). Avoids long-term maintenance of a parallel emission path.
- Q: `subscription.expiring` lead-time computation — what is `period_end`? → A: **`subscriptions.current_period_end`** (single source of truth per spec 018's data model). The dispatcher reads `current_period_end` for each active subscription, computes `period_end - lead_days`, and emits `subscription.expiring` at that trigger time. Do NOT compute from `created_at + billing_period_days` (drifts vs Stripe truth) and do NOT rely on Stripe's `invoice.finalized` event (that's 30 days before renewal, not configurable). One canonical field, one canonical read path.
