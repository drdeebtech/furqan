...
- **FR-013**: For each owned trigger, the system MUST consume the **event emitted by the owning spec** — except where this spec owns emission per the **round-2 clarification**: spec 023 consumes the existing `subscription.past_due` event (already emitted by spec 018's webhook) for the dunning / payment-failed flow, and **emits `subscription_expiring` and `absence_outcome` locally** (no upstream emitter exists for these as of 2026-06-19). Specs 018/021 may later take over emission if/when they ship those events; the dot.notation names already match. The expiry "continue?" prompt MUST be sent at exactly `period_end - N days` where N defaults to 7 and is admin-configurable via `platform_settings.subscription_expiring_lead_days`, and `period_end` is read from **`subscriptions.current_period_end`** (single source of truth per spec 018).
...
- Q: Consumed-events ownership — specs 018/021 never emitted `PaymentFailed`/`SubscriptionExpiring`/`AbsenceOutcome`. Who owns emission? → A: **Pragmatic split (Option C).** Spec 023 consumes the existing `subscription.past_due` event (already emitted by spec 018 webhook) for the dunning / payment-failed flow. Spec 023 emits `subscription_expiring` and `absence_outcome` locally (the two with no upstream emitter). Specs 018/021 can later take over emission if/when they ship those events; the dot.notation names already match the codebase convention. Unblocks US1/US5 webhook wiring (T012/T029/T030) without waiting on cross-team coordination.
...
- Q: T011b fallback month-close detector — when to ship? → A: **Ship only if spec 018 hasn't emitted `SubscriptionMonthClosed` by US1 ready date** (current plan). Minimal scope: the fallback exists to unblock, not as defense-in-depth. Once spec 018 ships the emitter, T011b is deleted (the task already says so). Avoids long-term maintenance of a parallel emission path.
- Q: `subscription_expiring` lead-time computation — what is `period_end`? → A: **`subscriptions.current_period_end`** (single source of truth per spec 018's data model). The dispatcher reads `current_period_end` for each active subscription, computes `period_end - lead_days`, and emits `subscription_expiring` at that trigger time. Do NOT compute from `created_at + billing_period_days` (drifts vs Stripe truth) and do NOT rely on Stripe's `invoice.finalized` event (that's 30 days before renewal, not configurable). One canonical field, one canonical read path.
