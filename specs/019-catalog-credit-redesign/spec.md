...
- **Individual hifz hour bundling**: individual hifz is priced at $10/hour (flexible, not fixed tiers). How hours map to discrete catalog tiers vs. a continuous hour count is a modeling decision — see [NEEDS CLARIFICATION] below.
...
- **FR-003**: System MUST seed the six captured price tiers: **group** hifz at 60-minute sessions — 4 sessions/month = $12, 6 = $15, 8 = $20; and **individual** hifz priced by hours at **$10/hour** (flexible, not fixed tiers). The per-hour rate and group tier prices MUST be admin-editable settings.
...
- **[NEEDS CLARIFICATION]**: Individual-hifz hour bundling — does the catalog expose discrete individual tiers (e.g., 4/6/8 hours) computed from the $10/hour rate, or a continuous student-chosen hour count billed at the rate? (Assumption: discrete admin-defined hour bundles, to keep parity with the six-tier model and spec 018's plan-per-tier mirror.)
